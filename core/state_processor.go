...
func (p *ParallelStateProcessor) hasInvalidReference(slotDB *state.StateDB,
	...
	for addr, storageReferred := range refDetail.KVChangeReferred {
		...
		for keyReferred := range storageReferred {
			if _, exist := keysChanged[keyReferred]; !exist {
				log.Info("Unconfirmed check, KV referred, but addr not exist in keysChanged",
					"txIndex", txIndex, " baseTxIndex", slotDB.BaseTxIndex(),
					"conflict TxIndex", changeList.TxIndex, "addr", addr)
				return true // fixme: to check the value
			}
		}
	}
	return false
...
func (p *ParallelStateProcessor) runShadowSlotLoop(slotIndex int) {
	...
	for {
		...
		log.Debug("runShadowSlotLoop to confirm the TxResult from master slot", "Slot", slotIndex, "txIndex", unconfirmedResult.txReq.txIndex)
		confirmedResult := p.executeInShadowSlot(slotIndex, unconfirmedResult)

		log.Debug("runShadowSlotLoop the TxReq is done", "Slot", slotIndex, "err", confirmedResult.err)
