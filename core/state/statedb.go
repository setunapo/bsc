				...
				if _, ok := db.parallel.stateChangesInSlot[addr]; ok {
					...
					if val, exist := obj.dirtyStorage[key]; exist {
						...
						if _, ok := s.parallel.unconfirmedRefList[db.txIndex].KVChangeReferred[addr]; !ok {
							s.parallel.unconfirmedRefList[db.txIndex].KVChangeReferred[addr] = make(Storage)
						}
						s.parallel.unconfirmedRefList[db.txIndex].KVChangeReferred[addr][key] = val
						return val, true
					}
					if val, exist := obj.pendingStorage[key]; exist { // fixme: concurrent access,
...
func (s *StateDB) CopyForSlot() *StateDB {
	parallel := ParallelState{
		...
		unconfirmedRefList:         make(map[int]UnconfirmedReferDetail, 100),
		...
		balanceChangesInSlot:       make(map[common.Address]*big.Int, defaultNumOfSlots),
