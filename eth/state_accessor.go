...
func (eth *Ethereum) StateAtBlock(block *types.Block, reexec uint64, base *state.StateDB, checkLive bool, preferDisk bool) (statedb *state.StateDB, err error) {
	...
	if base != nil {
		if preferDisk {
			...
			if statedb, err = state.NewWithEpoch(eth.blockchain.Config(), block.Number(), block.Root(), database, nil, eth.blockchain.ShadowNodeTree()); err == nil {
		...
		if !checkLive {
			statedb, err = state.NewWithEpoch(eth.blockchain.Config(), current.Number(), current.Root(), database, nil, eth.blockchain.ShadowNodeTree())
			if err == nil {
				return statedb, nil
			}
		}
	...
	for current.NumberU64() < origin {
		...
		statedb, err = state.NewWithEpoch(eth.blockchain.Config(), current.Number(), root, database, nil, eth.blockchain.ShadowNodeTree())
		if err != nil {
