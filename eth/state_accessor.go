...
func (eth *Ethereum) StateAtBlock(block *types.Block, reexec uint64, base *state.StateDB, checkLive bool, preferDisk bool) (statedb *state.StateDB, err error) {
	...
	if base != nil {
		if preferDisk {
			...
			if statedb, err = state.NewWithEpoch(block.Root(), database, nil, types.GetStateEpoch(eth.blockchain.Config(), block.Number())); err == nil {
		...
		if !checkLive {
			statedb, err = state.NewWithEpoch(current.Root(), database, nil, types.GetStateEpoch(eth.blockchain.Config(), current.Number()))
			if err == nil {
				return statedb, nil
			}
		}
	...
	for current.NumberU64() < origin {
		...
		statedb, err = state.NewWithEpoch(root, database, nil, types.GetStateEpoch(eth.blockchain.Config(), current.Number()))
		if err != nil {
